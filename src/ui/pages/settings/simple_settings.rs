    ...
    pub fn view<'a>(&'a self, config: &'a AppConfig) -> Element<'a, SimpleSettingsMessage> {
        ...
        content =
            content.push(
                container(
                    column()
                        .push(text("Default LLM Profile").size(16).class(
        ...
        for (profile_name, profile) in &config.profiles {
            content = content.push(self.profile_card(profile_name, profile, &config.default));
        }
        ...
        // Back button
        content =
            content.push(row().push(widget::Space::with_width(Length::Fill)).push(
                button::standard("Back to Chat").on_press(SimpleSettingsMessage::BackToMain),
            ));
            ...
            row()
                .push(
                    column()
                        ...
                        .push(text(format!("Model: {}", profile.model)).size(12).class(
                            cosmic::style::Text::Color(
                                theme::active().cosmic().palette.neutral_6.into(),
                            ),
                        ))
                        .push(
                            text(format!("Endpoint: {}", profile.endpoint))
                                .size(12)
                                .class(cosmic::style::Text::Color(
                                    theme::active().cosmic().palette.neutral_6.into(),
                                )),
    ...
    fn add_profile_section<'a>(&'a self) -> Element<'a, SimpleSettingsMessage> {
        container(
            column()
                ...
                .push(row().push(widget::Space::with_width(Length::Fill)).push(
                    button::suggested("Add Profile").on_press(SimpleSettingsMessage::AddNewProfile),
                ))
                .spacing(12),
        )
        .padding(16)
