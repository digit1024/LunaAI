    ...
    cosmic::widget::column::with_capacity(3)
        ...
        .push(
            ...
            if app.available_mcp_tools.is_empty() {
                ...
                let mut tool_list =
                    cosmic::widget::column::with_capacity(app.available_mcp_tools.len()).spacing(4);
                ...
                for tool in &app.available_mcp_tools {
                    ...
                    let tool_row = cosmic::widget::container(
                        cosmic::widget::column::with_capacity(3)
                            .push(
                                cosmic::widget::row::with_capacity(2)
                                    .push(cosmic::widget::toggler(is_enabled).on_toggle(
                                        |enabled| Message::ToggleTool(tool.name.clone(), enabled),
                                    ))
                                    .push(cosmic::widget::text(&tool.name).size(14).class(
                                        if is_enabled {
                            ...
                            .push(cosmic::widget::text(&tool.description).size(12).class(
                ...
                cosmic::widget::scrollable(tool_list).into()
            },
        )
        .spacing(8)
        .into()
