...
//!
//! Uses backend and model from ModelPreset. OpenAI-compatible  use cl100k_base.

use crate::config::{LlmProfile, ModelPreset};
use crate::llm::Message;

/// Tokenizer type based on model backend
#[derive(Debug, Clone)]
pub enum TokenizerType {
    /// OpenAI and OpenAI-compatible models (cl100k_base)
    Cl100kBase,
...
impl TokenCounter {
    /// Create a new token counter for the given model preset
    pub fn new(preset: &ModelPreset) -> Self {
        let tokenizer_type = Self::detect_tokenizer(preset);
    ...
    pub fn get_context_limit(&self, preset: &ModelPreset) -> usize {
        if let Some(configured_size) = preset.context_window_size {
            return configured_size;
        }

        let model_lower = preset.model.to_lowercase();
    ...
    pub fn get_safe_context_limit(&self, preset: &ModelPreset) -> usize {
        let limit = self.get_context_limit(preset);
    ...
    /// Pass preset for context limit and profile for threshold.
    pub fn get_summarize_threshold_tokens(&self, preset: &ModelPreset, profile: &LlmProfile) -> usize {
        let context_limit = self.get_context_limit(preset);
...
mod tests {
    ...
    fn test_context_limits() {
        ...
        let counter = TokenCounter::new(&preset);
        let limit = counter.get_context_limit(&preset);
        assert_eq!(limit, 128_000);
        let safe_limit = counter.get_safe_context_limit(&preset);
        assert_eq!(safe_limit, 102_400);
    }
}
